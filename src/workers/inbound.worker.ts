...
async function sendReplyAsChunks(
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    try {
      ...
      logger.error(
        { err, tenant: tenant.slug, waId, chunkIndex: i },
        "chunk send failed",
      );
    }
  }
...
const worker = new Worker<InboundJob>(
  ...
  async (job) => {
    ...
    // Trava por lead: dois turnos simultâneos do MESMO lead (mensagens muito
    // rápidas + concorrência do worker) bagunçam histórico e duplicam resposta.
    // Se já tem turno rodando, devolve o job pra fila com um pequeno atraso.
    ...
    if (locked !== "OK") {
      ...
      logger.debug(
        { tenant: tenant.slug, waId },
        "turno em andamento — job reenfileirado",
      );
      return;
    }
    ...
    try {
      ...
      if (!text) {
        logger.debug(
          { tenant: tenant.slug, waId },
          "empty buffer after debounce; skip",
        );
        return;
      }
      ...
      try {
          ...
          logger.info(
            {
              ...
              retry: !!result.retryAfterMs,
            },
            "turn.sent",
          );
        } else {
          logger.info(
            { tenant: tenant.slug, waId, state: result.newState },
            "turn.no-reply",
          );
        }
          ...
          await retryTurnQueue
            ...
            .catch((err) => {
              if (!String(err?.message ?? "").includes("already exists")) {
                logger.error(
                  { err, tenant: tenant.slug, waId },
                  "failed to enqueue retry-turn",
                );
              }
            });
          logger.info(
        ...
        if (
          result.replyText &&
          !result.closedReason &&
          !TERMINAL_STATES.includes(result.newState)
        ) {
          await scheduleFirstFollowup(tenantId, waId);
          logger.debug(
            { tenant: tenant.slug, waId },
            "followup stage 1 scheduled",
          );
        }
      } catch (err) {
        ...
        const lead = await getLead(tenantId, waId);
        const fallback =
          "Opa, tive um probleminha técnico aqui 😅 Pode repetir, por favor?";
        if (lead) {
          await logMessage(lead.id, "out", "assistant", fallback).catch(
            () => undefined,
          );
        }
