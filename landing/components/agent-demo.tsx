...
const SCENARIOS: Array<{ id: Scenario; icon: typeof Building2; title: string; desc: string }> = [
  { id: "imovel", icon: Building2, title: "Quero comprar um apartamento", desc: "Você é o cliente da imobiliária" },
  { id: "consorcio", icon: Landmark, title: "Quero investir em consórcio", desc: "Você é o lead do consultor" },
  { id: "cetico", icon: ShieldQuestion, title: "Sou cético, me convença", desc: "Jogue as objeções mais difíceis" },
];
...
export function AgentDemo() {
  ...
  async function applyTurn(next: TurnResponse) {
    ...
    setScore(next.score);
    pushEvents(next);
    // as respostas entram uma a uma, com "digitando…" entre elas
    for (const [i, r] of next.replies.entries()) {
      setTyping(true);
      await new Promise((res) => setTimeout(res, Math.min(400 + r.length * 12, 1800)));
      setTyping(false);
      setMsgs((m) => [...m, { from: "stella", text: r }]);
      if (i < next.replies.length - 1) await new Promise((res) => setTimeout(res, 250));
    }
    setSuggestions(next.done ? [] : next.suggestions);
  ...
  async function start(scenario: Scenario) {
    ...
    try {
      ...
      setPhase("live");
      setMsgs([{ from: "me", text: data.opener! }]);
      prevSlots.current = {};
      prevScore.current = 0;
      ...
      <div className="mx-auto max-w-6xl px-6">
        ...
        {phase === "pick" ? (
          <div className="mx-auto mt-12 max-w-3xl">
            <p className="mb-4 text-center text-[13px] text-ink-muted">Quem você quer ser nessa conversa?</p>
            <div className="grid gap-3 sm:grid-cols-3">
              {SCENARIOS.map(({ id, icon: Icon, title, desc }) => (
                <button
                  ...
                  disabled={busy}
                  onClick={() => start(id)}
                  className="group rounded-xl border border-line bg-canvas-surface p-5 text-left transition-all duration-200 hover:-translate-y-0.5 hover:border-accent-bronze/40 hover:shadow-glow-bronze disabled:opacity-50"
                >
                  ...
                  <span className="mt-3 flex items-center gap-1 text-[12px] text-accent-bronze-soft opacity-0 transition-opacity group-hover:opacity-100">
            ...
            {busy && <p className="mt-4 text-center text-[12px] text-ink-muted">Chamando a Stella…</p>}
          </div>
        ) : (
