...
import { Settings2, Hand } from "lucide-react";

// Lead aparece no board se estiver aberto, agendado ou em atendimento humano.
// Leads "mortos" (sem interesse / sem resposta / adiado) saem do board.
function onBoard(l: Lead): boolean {
  return l.status === "open" || l.closed_reason === "scheduled" || l.closed_reason === "handoff";
}
...
export function LeadsBoard({
  ...
  // poll a cada 5s pra refletir o avanço automático da IA no board.
  useEffect(() => {
  ...
  function handleDrop(stageId: number, e: React.DragEvent) {
    ...
    // otimista: move o card na hora; reverte via refetch se a API falhar.
    setLeads((prev) =>
      prev.map((l) =>
        l.wa_id === waId
          ? { ...l, pipeline_stage_id: stageId, stage_manual: mapped == null, state: mapped ?? l.state }
          : l,
      ),
    );
    fetch(`/api/leads/${waId}/move`, {
      ...
      body: JSON.stringify({ to_stage_id: stageId }),
    })
      .then((r) => {
                ...
                {col.length === 0 ? (
                  ...
                  <div className="flex flex-col gap-2">
                    {col.map((l) => (
                      ...
                      >
                        <LeadCard lead={l} onClick={() => setSelected(l.wa_id)} />
                      </div>
                    ))}
                  </div>
                )}
